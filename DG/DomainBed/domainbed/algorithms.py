...
class RCERM(Algorithm): # Refined_Contrastive_ERM
    """
    Refined Contrastive Empirical Risk Minimization (RCERM)
    """
...
################################ Code required for ERMR ################################ 

class ERMR(Algorithm): # ERM_with_Refined_features
    """
    Empirical Risk Minimization with Refined Features (ERMR)
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ERMR, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        for id_c in range(nclass): # loop over classes
            for id_d in range(ndomains): # loop over domains
                ...
                positive_queue=queue_var.get_pos_queues(id_c,id_d,train_queues)
                #### Gated Fusion + Attention based refinement and augmentations
                k=self.get_augmented_batch(q,positive_queue)
                
                all_x = torch.cat((all_x, k), 0) # using augmented features for CE loss, for ERM
                all_y = torch.cat((all_y, label_tensor), 0) # corresponding labels 
...
################################ Code required for ERMRM1 ################################ 

class ERMRM1(Algorithm): # ERM_with_Refined_features
    """
    Empirical Risk Minimization with Refined Features and Mean at Step 1(ERMRM1)
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ERMRM1, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
...
#                                         )
    ...
    def return_means(self,fp):
        ndoms=int(fp.size(0)/queue_sz)
        #print(ndoms)

        fp_means=None
        for dd in range(ndoms):
            if fp_means is not None:
                fp_means=torch.cat((fp_means,torch.mean(fp[dd*queue_sz:dd*queue_sz+queue_sz], 0, True)),0)
            else:
                fp_means=torch.mean(fp[dd*queue_sz:dd*queue_sz+queue_sz], 0, True)
        return fp_means
...
################################ Code required for ERMRM2 ################################ 

class ERMRM2(Algorithm): # ERM_with_Refined_features
    """
    Empirical Risk Minimization with Refined Features and Mean at Step 2(ERMRM2)
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ERMRM2, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
...
#                                         )
    ...
    def get_augmented_feature(self, fx,fp):
        fp_refined=self.gated_fusion_refinement(fx,fp)
        fp_refined = self.return_means(fp_refined)
        fxg,wx=self.refine_augment(fx,fp_refined)
        return fxg
...
################################ Code required for ERMRNG ################################ 

class ERMRNG(Algorithm): # ERM_with_Refined_features
    """
    Empirical Risk Minimization with Refined Features with No Gated Fusion (ERMRNG)
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ERMRNG, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
...
#                                         )
    ...
    def get_augmented_feature(self, fx,fp):
        ...
        fp_refined = self.return_means(fp)
        fxg,wx=self.refine_augment(fx,fp_refined)
        return fxg
...
################################ Code required for CLNR ################################ 

class CLNR(Algorithm): # Contrastive Loss with No Refined features
    """
    Contrastive Loss with No Refined features (CLNR)
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(CLNR, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
...
#                                         )
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        nclass=len(train_queues)
        ndomains=len(train_queues[0])
        loss_cl=0 # for accumulating the additive contrastive loss
        ...
        for id_c in range(nclass): # loop over classes
            for id_d in range(ndomains): # loop over domains
                ...
                random_ids = np.random.choice(positive_queue.size(0), size=q.size(0), replace=False)
                k=positive_queue[random_ids,:]
                
                
                # detach the key as we won't be backpropagating by the key encoder
                # not reqd as directly fetching from queue without any computation
                #k = k.detach()

                #l2 normalize
                q = torch.div(q,torch.norm(q,dim=1).reshape(-1,1))
                # no need to normalize as aleady done while storing into queue
                #k = torch.div(k,torch.norm(k,dim=1).reshape(-1,1))
                ...
                # k: embedding of the positive (augmented anchor)
                # negative_queue: embeddings in the memory queue
                loss_cl += queue_var.loss_function(q, k, negative_queue)
        ...
        return {'loss': loss.item(),'loss_ce': loss_ce.item(),'loss_cl': loss_cl.item()}
